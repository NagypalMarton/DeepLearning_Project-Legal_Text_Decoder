...
def main():
	...
	logger.info(f"RAW input: {raw_dir}")
	logger.info(f"Processed output: {processed_dir}")
	logger.info(f"Reports dir (metrics/plots): {features_dir}")
	...
	logger.info(f"Loaded {len(df_raw)} rows from {raw_csv}")
	...
	logger.info(f"Rows after removing empty text/label: {after_missing} (removed {before - after_missing})")
	...
	after_dedup = len(df)
	logger.info(f"Rows after deduplication: {after_dedup} (removed {before_dedup - after_dedup})")
	...
	if before - after_missing > 0:
		...
		logger.info(f"Saved removed missing rows to {missing_file}")
	if before_dedup - after_dedup > 0:
		...
		logger.info(f"Saved removed duplicates to {dup_file}")
	...
	if target_column in df.columns and len(df) > 10:
		...
		logger.info(f"Train set: {len(train_df)}, Val set: {len(val_df)}, Test set: {len(test_df)}")
		...
		for name, split_df in [('train', train_df), ('val', val_df), ('test', test_df)]:
			...
			logger.info(f"Saved {name}.csv with stats -> {out_csv}")
		...
		if enable_embeddings:
			...
			logger.info("Embeddings disabled (set ENABLE_EMBEDDINGS=true to enable).")
	else:
